...
class JeuGraphique(object):
    ...
    def __init__(self, lecteur_jeu, nom_partie, titre="Pacman'IUT'O", size=(860, 640),
                 couleur=(209, 238, 238),
    ...
    def get_images(self, prefixe_image="./images"):
        ...
        for i in range(7):
            if os.path.isfile(os.path.join(prefixe_image, 'tresor' + str(i) + '.png')):
                s = pygame.image.load(os.path.join(
                    prefixe_image, 'tresor' + str(i) + '.png'))
            else:
                s = None
    ...
    def maj_surfaces(self, dico_entree, dico_sortie, taille):
        for (cle, img) in dico_entree.items():
            dico_sortie[cle] = pygame.transform.smoothscale(
                img, (taille, taille))
    ...
    def dessiner_case2(self, la_case, lin, col, voisinage, coul_bord=(131, 0, 101), coul_fond=None):
        ...
        if not dessiner:
            if fond:
                pygame.draw.rect(self.surface, coul_fond,
                                 (x, y, self.delta, self.delta))
            return
    ...
    def surface_case(self, la_case):
        ...
        coord = [(dist, dist), (dist, self.delta - (self.delta // 2 + dist)),
                 (self.delta - (self.delta // 2 + dist),
                  self.delta - (self.delta // 2 + dist)),
                 (self.delta - (self.delta // 2 + dist),
                  dist), (self.delta//4+dist, dist),
                 (self.delta//4+dist, self.delta - (self.delta // 2 + dist))]

        if objet != ' ':
            surf_carte.blit(
                self.surf_objets[objet], (self.delta // 4, self.delta // 4))
        for pac in pacmans:
    ...
    def affiche_joueurs(self, ligne, couleur=None):
        ...
        for le_joueur in classement:
            ...
            for un_obj in jeu._fonction_21(le_joueur):
                self.surface.blit(
                    self.surf_objets_min[un_obj], (posx-self.delta//2, posy+(nb_obj*self.delta//2)))
                nb_obj += 1
            texte = font.render(
    ...
    def affiche_info(self):
        ...
        if nb_tours <= 1:
            pluriel = ""
        self.affiche_message(3, "il reste " + str(nb_tours) +
                             " tour" + pluriel + " de jeu", [])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--nom_partie", dest="nom_partie",
                        help="nom de la partie", type=str, default='Non fournie')
