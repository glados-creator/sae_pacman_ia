    ...
    while rep != 'Q':
      ...
      if recup != None:
        for id_joueur, actions in recup.items():
          ...
          msg = self.table_clients.le_jeu.executer_deplacer_pacman(
              coul, actions[0])
          if msg != '':
            print(msg, end='')
          self.table_clients.envoyer_jeu(AFFICHEUR, msg)
          msg = self.table_clients.le_jeu.executer_deplacer_fantome(
              coul, actions[1])
          if msg != '':
            print(msg, end='')
          self.table_clients.envoyer_jeu(AFFICHEUR, msg)
...
if __name__ == '__main__':
  ...
  parser.add_argument("--nom_partie",
                      dest="nom_partie",
                      help="nom de la partie",
                      type=str,
                      default='score.csv')
  parser.add_argument("--duree",
                      dest="duree",
                      help="nombre de tours de la partie",
                      type=int,
                      default=100)
  parser.add_argument("--map",
                      dest="map",
                      help="fichier contenant la map",
                      type=str,
                      default='cartes/test1.txt')
  ...
  ecouteur.start()
  le_jeu = JeuThread(ecouteur, table_clients, args.duree, args.nom_partie,
                     args.map)
  le_jeu.start()
